...
| OWASP 집중 | **A01** 접근제어, **A03** 인젝션(SQLi 등), **A07** 인증 취약점(패턴·휴리스틱) |
...
- 모든 OWASP Top 10을 동일 깊이로 구현 — 문서상 로드맵에 두고 단계 확장
...
- SQLi, 커맨드 인젝션 일부 패턴, XPath/LDAP 등은 **후속 단계**로 분리  
- 구현: 정규식·키워드·길이 이상, 인코딩 디코드 후 재검사(이중 인코딩 대응 일부)

### 4.2 A01 — 접근 제어

- 경로 순회(`../`), 관리자 경로 노출, IDOR 후보(순차 ID·과도한 객체 참조 패턴) 휴리스틱  
- **주의:** 실제 IDOR는 문맥·세션 필요 → LLM/로그는 **보조**, 최종 판단은 보수적 정책

### 4.3 A07 — 인증 취약점

- 취약한 쿠키 플래그 추정, 브루트포스 패턴(율리밋과 연계), 기본 자격증명·하드코딩 힌트 탐지  
- 로그인 엔드포인트 집중 모니터링 옵션
...
**스키마(초안):** 타임스탬프, 클라이언트 IP(해시 가능), 메서드, 경로, OWASP 태그, 판정, 규칙 ID, LLM 사용 여부, 지연(ms), 리미디에이션 요약(텍스트)
...
- `GET /api/dashboard/summary` — 기간별 차단/허용, OWASP 분포  
...
- 화면: 최근 이벤트 테이블, OWASP 파이 차트(또는 막대), 차단율 KPI, AI 제안 카드  
...
ai-security-system/
  ...
  detector.py             # OWASP 1차·2차 호출 조율
  proxy/                  # 업스트림 전달, 헤더 처리 (선택 분리)
  owasp/                  # a01.py, a03.py, a07.py 등 모듈화 (선택)
...
| 단계 | 기간(가이드) | 산출물 |
|------|----------------|--------|
| **Phase 0** | 완료 | GitHub 저장소, 브랜딩, Cursor 규칙, 본 계획서 |
| **Phase 1** | 1–2주 | `main.py` 프록시 뼈대, 환경변수, 헬스체크, 업스트림 포워딩 검증 |
| **Phase 2** | 1–2주 | `detector.py` A03 시그니처, 차단 응답, SQLite 로깅 |
| **Phase 3** | 1–2주 | LLM 연동, 의심 요청만 호출, 분류 JSON 파싱 |
| **Phase 4** | 1주 | A01/A07 최소 휴리스틱 통합, 정책 플래그 정리 |
| **Phase 5** | 1–2주 | 리미디에이션 파이프라인(프롬프트 + 저장 + API) |
| **Phase 6** | 1–2주 | 대시보드 API + 기본 웹 UI, 차트/필터 |
| **Phase 7** | 마무리 | 데모 시나리오, README 실행법, 한계·향후 과제 정리 |

일정은 병행 가능(예: Phase 3와 5 일부 병렬).
...
2. 전형적 SQLi 쿼리 — 1차 차단 또는 LLM 확인 후 차단  
3. 경로 순회 시도 — A01 태그 및 로그  
4. 반복 로그인 실패 — A07/율리밋(구현 시)  
5. 대시보드에서 위 이벤트가 OWASP 분류·차단율에 반영되는지 확인  
...
- [ ] A03 중심 **자동 탐지**가 동작하고, 이벤트가 **DB에 기록**된다.  
...
| 날짜 | 내용 |
|------|------|
| 2025-03-24 | 초안 작성 — 요구사항·아키텍처·로드맵 통합 |
