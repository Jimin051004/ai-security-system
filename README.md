...
1. **Juice Shop** (Docker): 프로젝트 루트에서 `docker compose up -d` 후 `http://127.0.0.1:3000` (또는 LAN IP)으로 확인.
...
일부 리다이렉트가 업스트림 포트(3000)를 가리킬 수 있어, 데모 중에는 주소창이 기대한 호스트·포트를 유지하는지 확인하면 된다.
