...
이 저장소의 `docker-compose.yml`은 호스트의 **모든 인터페이스**에서 포트 **3000**을 연다.  
같은 네트워크의 다른 사람은 다음으로 **직접** Juice Shop에 접속할 수 있다.

- **직접 접속 URL:** `http://192.168.0.10:3000`  
...
```env
UPSTREAM_URL=http://192.168.0.20:3000
```
...
다른 사람이 끊기면 호스트 OS 방화벽에서 **TCP 3000**(직접 Juice Shop), **TCP 8080**(프록시) 허용이 필요할 수 있다.
...
| 목적 | URL |
|------|-----|
| Juice Shop 직접 (WAF 없음) | `http://192.168.0.10:3000` |
...
- **리다이렉트가 `localhost:3000`으로 감:** Juice Shop이 절대 URL을 줄 때 발생할 수 있다. 주소창을 다시 `http://<LAN-IP>:8080`(또는 3000)으로 맞추거나, 같은 호스트에서만 재현한다.
