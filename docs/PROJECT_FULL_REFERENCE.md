...
**기능:** **pytest** 자동 테스트. `conftest.py`에서 `UPSTREAM_URL`, `WAF_*` 기본값 설정.
...
| `test_app_health.py` | 앱 헬스/기본 응답 |
| `test_dashboard.py` | 대시보드 HTML, API, 차단 시 traffic `block_findings` |
| `test_detector_policy.py` | 심각도 파싱·필터·`all_findings` |
| `test_proxy_rewrite.py` | 업스트림 URL → 공개 호스트 치환, 응답 헤더 형식 |
| `test_traffic_log.py` | 기록·스냅샷·`/__waf` 미기록 |
| `test_waf_block_page.py` | 403 시 JSON vs HTML `Accept` 분기, Juice Shop 스타일 Accept |
| `test_a05_login_sqli.py` | JSON 로그인 바디 `admin@juice-sh.op' -` 등 **A05-SQL-010** |
